    ...
    public partial class Main : MetroForm
    {
        ...
        private void metroToggle1_Click(object sender, EventArgs e)
        {
            if (metroToggle1.Checked == false)
            {
                ...
                if (File.Exists(@"customMood.txt"))
                {
                    var customMood = File.ReadAllText(@"customMood.txt");
                    metroTextBox1.Text = customMood;
                }
                else
                {
                    metroTextBox1.Text = "I am currently playing a game.";
                    var createMood = File.Create(@"customMood.txt");
                    File.GetAccessControl(@"customMood.txt");
                    createMood.Close();
                    File.WriteAllText(@"customMood.txt", metroTextBox1.Text);
                }
                metroLabel9.Visible = true;
            }
            else
